...
function CatalogueInner() {
    ...
    // Taille sélectionnée par produit (clé = productId, valeur = taille choisie)
    ...
    // On ne fetch les favoris que si l'utilisateur est connecté, pour éviter
    // que le middleware redirige vers /sign-in (ce qui renvoie du HTML au
    // lieu de JSON et casse le parsing côté front).
    useEffect(() => {
        if (isSignedIn) {
            fetchFavorites();
        } else {
                ...
                {filteredProducts.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8">
                        {filteredProducts.map(product => {
                            ...
                            // Si le produit a des tailles, on exige qu'une soit choisie
                            // avant d'activer les boutons Ajouter/Acheter.
                                ...
                                >
                                    ...
                                    <div className="p-5">
                                                ...
                                                {product.sizes.map(size => {
                                                    ...
                                                    return (
                                                        <button
                                                            ...
                                                            className={`min-w-[28px] text-center px-2 py-1 text-[10px] font-medium uppercase tracking-wide rounded-md border transition ${
                                                                isSelected
                                                                    ? "border-[#3b82f6] bg-[#3b82f6] text-white"
                                                                    : "border-[#1e3a5f] text-[#8aabca] bg-[#0a1628] hover:border-[#3b82f6]/60"
                                                            }`}
                                                        >
                                                            {size}
                                                        </button>
                                        ...
                                        {sizeRequiredButMissing && (
                                            <p className="mt-2 text-[10px] text-[#f59e0b] uppercase tracking-wide">
