  ...
  return (
    <div className="group relative bg-white/90 backdrop-blur-sm rounded-3xl shadow-2xl overflow-hidden hover:shadow-pink-300/50 transition-all duration-500 border border-pink-100">
      <div className="relative h-80 bg-gradient-to-br from-purple-50 to-pink-50 p-8 flex items-center justify-center">
        <img
          src={perfume.imageUrl || "https://images.unsplash.com/photo-1541643600914-78b084683601?w=600"}
          alt={perfume.name}
      ...
      <div className="p-6">
        ...
        <div className="flex justify-between items-center mb-4">
          <div>
            <span className="text-3xl font-black text-purple-700">{perfume.price.toLocaleString()} TND</span>
          </div>
        ...
        <div className="flex flex-wrap gap-2">
          {perfume.style.slice(0, 3).map((s) => (
            <span key={s} className="text-xs bg-pink-100 text-pink-700 px-3 py-1 rounded-full">
              {s}
            </span>
          ))}
        </div>
